...
- **Analytics**: the PostHog project key (`phc_…`) is public by design and ships in client JS; only private (`phx_…`) keys are secrets. Config is cookieless: memory persistence, no autocapture, no session recording, EU host, loaded deferred so analytics never costs paint. Named events: `cv_pdf_download` (variant, pdf_theme), `cv_variant_toggle` (variant), `cv_print` (variant), `email_click`, `social_click` (network), `project_click` (url), `outbound_click` (url), `palette_open`, `palette_run` (item), `theme_toggle` (theme), `markdown_copy`, `open_chatgpt`, `open_claude`, `repo_table_use` (action, first-use-per-visit), `scroll_depth` (depth, /cv + blog only), `not_found` (path, referrer). All events auto-carry `page` + `theme`. Placement rule in AGENTS.md. Tracking disabled on localhost. Real-user web vitals via `capture_performance: { web_vitals: true }`.
